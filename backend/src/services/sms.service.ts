...
export async function sendPronosticToActiveSubscribers(pronosticId: number) {
  ...
  const template = templateSetting?.value ?? 'Pronostic du {date}: Base {base}';

  const tierce = Array.isArray(pronostic.tierce) ? (pronostic.tierce as string[]).join(' - ') : '';
  const quarte = Array.isArray(pronostic.quarte) ? (pronostic.quarte as string[]).join(' - ') : '';
  const quinte = Array.isArray(pronostic.quinte) ? (pronostic.quinte as string[]).join(' - ') : '';

  const message = template
    .replace('{date}', format(pronostic.date, 'dd/MM/yyyy', { locale: fr }))
    .replace('{base}', pronostic.baseHorse ?? '')
    .replace('{tierce}', tierce)
    .replace('{quarte}', quarte)
    .replace('{quinte}', quinte)
    .replace('{outsider}', pronostic.outsider ?? '')
    .replace('{score}', String(pronostic.confidenceScore));
  ...
  const results = await Promise.all(
    activeSubscribers.map((sub) => sendSmsToSubscriber(sub.id, message))
  );

  await prisma.pronostic.update({ where: { id: pronosticId }, data: { isSent: true } });

  const sent = results.filter((r) => r.success).length;
  logger.info(`Pronostic sent to ${sent}/${activeSubscribers.length} subscribers`);
    ...
    switch (command) {
      case 'PRONO': {
        ...
        if (!prono) {
          ...
          const tierce = Array.isArray(prono.tierce) ? (prono.tierce as string[]).join(' - ') : '';
          responseMessage = `🏇 Pronostic PMU\n🐴 Base: ${prono.baseHorse}\n🎯 Tiercé: ${tierce}\n✅ Confiance: ${prono.confidenceScore}/100`;
        }
        break;
      }
      case 'RESULTAT': {
...
export async function checkExpiringSubscriptions() {
  ...
  const expiring = await prisma.subscriber.findMany({
    where: {
      status: 'ACTIVE',
      endDate: { gte: tomorrow, lte: in2days },
    },
  });

  for (const sub of expiring) {
    const daysLeft = Math.ceil((sub.endDate.getTime() - Date.now()) / 86400000);
    const message = `⚠️ Votre abonnement PMU-PRONO expire dans ${daysLeft} jour(s). Renouvelez dès maintenant pour continuer à recevoir nos pronostics.`;
    await sendSmsToSubscriber(sub.id, message);
  }

  logger.info(`Expiry alerts sent to ${expiring.length} subscribers`);
}
